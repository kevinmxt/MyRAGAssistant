...
- `me.maxt.rag.web.config.AppConfig` — 配置容器（load() 装配 11 个配置节 record，节访问器取用）
- `me.maxt.rag.web.config.ConfigBinder` — 通用配置绑定器（@Key 注解一处声明 json 路径/env/默认值，优先级链装配，见 CONTEXT.md）
- `me.maxt.rag.web.service.vector.MilvusSession` — 向量库会话（连接生命周期、降级/重连、DEGRADED↔CONNECTED 原子切换，见 CONTEXT.md）
- `me.maxt.rag.web.service.vector.RetrievalPipeline` — 检索管线（唯一检索事实源，P/E/M 三通道 + 统一精排后处理，见 CONTEXT.md）
- `me.maxt.rag.web.service.RAGService` — 对话门面（composePrompt 组装参考资料 + 答案生成，检索移交 RetrievalPipeline）
- `me.maxt.rag.web.service.model.ModelRepository` — 模型仓库（镜像回退、原子落盘、状态记账）
